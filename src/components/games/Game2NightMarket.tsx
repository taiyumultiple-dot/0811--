 ...
 * （國語出題、台語漢字＋臺羅拼音打磚塊、每局隨機抽10題、答錯不扣命只扣分、
 * 球掉出才扣生命的規則），檔案放在 public/games/taigi-brick-breaker.html，
 * 透過 iframe 載入，不需要額外的建置流程即可運作。
 *
 * 舊版 React 實作備份在 Game2NightMarket.legacy.tsx.bak，
 * 之後若要切換回舊版，可以直接把該檔內容還原成 Game2NightMarket.tsx。
 */
export default function Game2NightMarket({
  onNext,
  onHome,
}: {
  onNext: () => void;
  ...
  useGameScoreBridge('game2', (titles) => setNewBadges(titles));
